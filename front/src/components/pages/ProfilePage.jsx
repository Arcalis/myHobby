...
export function ProfilePage() {
    ...
    const favoriteEvents = mockEvents.slice(2, 4);
    const myEvents = user.role === 'organizer' || user.role === 'admin' ? mockEvents.slice(0, 3) : [];
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="mb-6">
            ...
            {(user.role === 'organizer' || user.role === 'admin') && (<TabsTrigger value="my-events" className="gap-2">
          ...
          <TabsContent value="registered" className="space-y-4">
            ...
            {registeredEvents.length === 0 ? (<div className="bg-card border border-border rounded-lg p-12 text-center">
                ...
                {registeredEvents.map((event) => (<div key={event.id} className="bg-card border border-border rounded-lg p-6">
          ...
          <TabsContent value="favorites" className="space-y-4">
            ...
            {favoriteEvents.length === 0 ? (<div className="bg-card border border-border rounded-lg p-12 text-center">
                <p className="text-muted-foreground mb-4">У вас пока нет избранных мероприятий</p>
                <Link to="/events">
                  <Button>Найти мероприятия</Button>
                </Link>
              </div>) : (<div className="grid md:grid-cols-2 gap-4">
                {favoriteEvents.map((event) => (<div key={event.id} className="bg-card border border-border rounded-lg p-6 space-y-3">
          ...
          {(user.role === 'organizer' || user.role === 'admin') && (<TabsContent value="my-events" className="space-y-4">
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="space-y-4">
                {myEvents.map((event) => (<div key={event.id} className="bg-card border border-border rounded-lg p-6">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 space-y-3">
                        <div className="flex items-start gap-3">
                          <span className="px-2 py-1 bg-primary/10 text-primary rounded text-xs font-medium">
